...
def threshold_position(lux, max_lux=25):
    """Map lux values to a 0–100 visual position for threshold bars."""
    if lux <= 0:
        return 0
    if lux >= max_lux:
        return 100
    return (lux / max_lux) * 100
...
.threshold-track.urban {
  background: linear-gradient(to right,
    #7A7A7A 0%, #7A7A7A 12%,
    var(--white) 12%, var(--white) 60%,
    var(--yellow) 60%, var(--yellow) 100%);
}

.threshold-track.green {
  background: linear-gradient(to right,
    #7A7A7A 0%, #7A7A7A 8%,
    var(--white) 8%, var(--white) 40%,
    var(--yellow) 40%, var(--yellow) 100%);
}
...
.threshold-labels span:nth-child(1) { color: var(--grey); text-align:left; }
.threshold-labels span:nth-child(2) { color: var(--white); text-align:center; }
.threshold-labels span:nth-child(3) { color: var(--yellow); text-align:right; }
...
if uploaded_file is not None:
    ...
    with right_col:
        urban_marker_position = threshold_position(prediction, max_lux=25)
        green_marker_position = threshold_position(prediction, max_lux=25)

        st.markdown(
            f'<div class="card"><div class="section-title"><span class="yellow">☼</span> Estimated Illuminance</div><div class="lux-number">{prediction:.2f} <span class="unit">lx</span></div><div class="small-label">Horizontal illuminance</div><div class="threshold-bars">
                    <div class="threshold-row">
                        <div class="threshold-head">
                            <span class="threshold-title">Urban Area · P1</span>
                            <span class="threshold-values">0 · 3 · 15 · 25+ lx</span>
                        </div>
                        <div class="threshold-track urban">
                            <div class="threshold-marker" style="left:calc({urban_marker_position}% - 11px);"></div>
                        </div>
                        <div class="threshold-labels">
                            <span>Underlit</span><span>Properly lit</span><span>Overlit</span>
                        </div>
                    </div>
                    <div class="threshold-row">
                        <div class="threshold-head">
                            <span class="threshold-title">Green Area · P2</span>
                            <span class="threshold-values">0 · 2 · 10 · 25+ lx</span>
                        </div>
                        <div class="threshold-track green">
                            <div class="threshold-marker" style="left:calc({green_marker_position}% - 11px);"></div>
                        </div>
                        <div class="threshold-labels">
                            <span>Underlit</span><span>Properly lit</span><span>Overlit</span>
                        </div>
